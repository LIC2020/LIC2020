...
parser.add_argument("--train_data_file", type=str, default='xgb_data2/train_cands.csv')
parser.add_argument("--valid_data_file", type=str, default='xgb_data2/valid_cands.csv')
parser.add_argument("--test_data_file", type=str, default='xgb_data2/test1_cands.csv')
parser.add_argument("--x_train_file", type=str, default='xgb_data2/x_train.csv')
parser.add_argument("--x_valid_file", type=str, default='xgb_data2/x_valid.csv')
parser.add_argument("--x_test_file", type=str, default='xgb_data2/x_test.csv')
parser.add_argument("--y_train_file", type=str, default='xgb_data2/y_train.csv')
parser.add_argument("--y_valid_file", type=str, default='xgb_data2/y_valid.csv')
...
parser.add_argument("--pred_data_file", type=str, default='model44444/results_xgb.csv')
parser.add_argument("--model_path", type=str, default="./model44444")
...
def train_bagging():
    ...
    bag_clf = BaggingClassifier(DecisionTreeClassifier(), n_estimators=200, max_samples=1.0, bootstrap=True, n_jobs=-1)

    bag_clf.fit(x_train, y_train)
    y_pred = bag_clf.predict(x_valid)
    pred_score = accuracy_score(y_pred, y_valid)
    print(pred_score)

    p_test = y_pred

    test_df = pd.read_csv(args.valid_data_file, sep="\t", header=0, encoding="utf-8")

    test_df["score"] = p_test.ravel()
    logger.info("write pred_result")
    test_df.to_csv(args.pred_data_file,
                   header=True, index=False, encoding='utf-8', sep="\t")
    logger.info("write pred_result done")
    logger.info("write final_pred_result")
    test_df = pd.read_csv(args.pred_data_file, sep="\t", header=0)
    test_df_pred = test_df.groupby("qid")["score"].max()
    res_df = pd.merge(
        test_df,
        test_df_pred,
        on=["qid", "score"], how="inner", copy=False)
    res_df.to_csv(args.pred_data_file + "_final",
                  header=True, index=False, encoding='utf-8', sep="\t")
    ...
    json.dump(res_json, open(args.pred_data_file + "_final.json", "w"),
              ensure_ascii=False, indent=4)
